---
title: "Assignment"
output: html_document
runtime: shiny
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
library(shiny)
library(fivethirtyeight)
library(tidyverse)
library(ggiraph)
knitr::opts_chunk$set(echo = F)
```


### A tool for exploratory visualization of insurance data

```{r}
#For a list of control widget: https://shiny.rstudio.com/tutorial/written-tutorial/lesson3/
div(style="display:inline-block", selectInput( inputId = "xaxis", "X variable:", names(bad_drivers), selected = 'perc_speeding'))
div(style="display:inline-block", selectInput( inputId = "yaxis", "Y variable:", names(bad_drivers), selected = 'perc_alcohol'))
div(style="display:inline-block", selectInput( inputId = "size", "Dot size:", names(bad_drivers), selected = 'perc_alcohol'))
div(style="display:inline-block", textInput( inputId = "color", "Selected state:", value = 'Louisiana'))
```


```{r}
#read about `aes_string`. You want to use `aes_string` instead of `aes` since `selectInput` returns a character.
renderGirafe({
p1 <- ggplot(data = bad_drivers, aes_string(x = input$xaxis, y = input$yaxis, size= input$size, tooltip = 'state'))  +
    geom_point_interactive(aes(color = state == input$color)) +
  scale_color_discrete(guide = F) +
  ggtitle('Insurance premiums data')
girafe(ggobj = p1, width = .7)
})

```




