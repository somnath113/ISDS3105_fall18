...
```{r, echo=FALSE}
ggplot(police_killings, aes(age, urate)) +
...
```{r, echo=FALSE}
police_killings %>% 
  gather('race', 'percent', share_white:share_hispanic) %>% 
  group_by(race) %>% 
  summarise(avg  = mean(percent, na.rm = T))
```
...
```{r, echo=FALSE}
police_killings %>% 
  filter(age < 25 & gender == 'Male') %>% count(raceethnicity)
```
...
split(police_killings, police_killings$raceethnicity) %>% 
map(dt,  ~ ggplot() + geom_bar(aes(x = ...$gender)))
```
...
map(dt,  ~ ggplot(data = .x) + geom_bar(aes(x = gender)))
```
