...
select(iphoneSales, -year) %>% 
purrr::map(.x = ., .f = ~ mean(.x, na.rm = T))
#without anonymous
select(iphoneSales, -year) %>% 
purrr::map(.x = ., .f = mean, na.rm = T)
```
...
```{r}
#' Use map to produce 3 charts: Southern States, Mountain states, Others. 
