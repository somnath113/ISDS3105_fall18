...
```{r}
ggplot(data = sn_users, aes(x = year)) +
 geom_line(aes(y =  twitter, color = 'twitter' ) ) +
 geom_line(aes(y = facebook, color = 'fb')) +
...
gather(sn_users, key = 'socialNet', value = 'Mil.Users')
```
...
gather(untidyReview, key= "Attributes", value="Rating", reviewOverall:reviewValue) %>% 
  ggplot(aes(x = Rating)) +
 geom_bar() +
facet_wrap(c('reviewStayYear', 'Attributes'))
```
...
gather(iphoneSales, key = "Quarter", value = "Sales",-year , na.rm = TRUE) %>% str
gather(iphoneSales, ... ) %>% 
  ggplot() +
