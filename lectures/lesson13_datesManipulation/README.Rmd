...
dt %>% 
 mutate(fulldate = ymd_hms(paste(offensedate,offensetime))) %>%
 mutate(time = hms(offensetime)) %>% select(fulldate, time) %>% 
 mutate(offensetime_discrete = case_when(
 between(time, hms('22:00:00'), hms('24:00:00')) ~ 'night',
 between(time, hms('00:00:00'), hms('05:00:00')) ~ 'night',
 between(time, hms('05:00:00'), hms('10:30:00')) ~ 'morning',
 between(time, hms('10:30:00'), hms('14:00:00')) ~ 'day',     
 between(time, hms('14:00:00'), hms('17:30:00')) ~ 'afternoon',
    T ~ 'evening'     
 ))
```
