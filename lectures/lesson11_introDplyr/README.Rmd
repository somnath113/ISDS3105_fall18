...
```{r}
titanicDt %>% distinct(Age, Sex)
distinct(titanicDt, Age, Sex)
```
...
```{r}
titanicDt %>%  arrange(Class, Age, Sex)
```

2. Using `titanicDt`, arrange descending by `n`
```{r}
arrange(titanicDt, desc(n))
```
...
```{r}
beerDt %>%   mutate(liters = GallonsCapita * 3.78)
```

2. Using `socialNetwork_users` add a percent sign to the variable percent (use `paste0()`)
```{r}
socialNetwork_users %>% mutate(percent = paste0(percent, "%"))
```
...
#flights
#airlines
right_join(flights, airlines) %>% nrow
semi_join(flights, airlines) %>% nrow
full_join(flights, airlines) %>% nrow
full_join(flights, airlines) %>% nrow
semi_join(airlines, flights) %>% nrow
left_join(airlines, flights) %>% nrow
```
...
3. Calculate the average air_time for the flights by each carrier. Plot the result in a barchart.
