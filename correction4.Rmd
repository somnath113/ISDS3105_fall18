...
dateposted <- lubridate::dmy_hms(created_at)

ggplot(data = dt) +
  geom_histogram(aes(x = date, y = count(select(id)), xlab=wday(date) + " '" + year(date)))
...
income <- select(hate_crimes, median_house_inc, state)
top_n(income, 5)

mutate(income, state = tolower(state)) %>% 
  right_join(states, by = c('state' = 'region')) %>% 
      ggplot() +
      geom_polygon(aes(x = long, y = lat, group = group, fill = income), color = "white") + 
      coord_fixed(1.3) +
      labs(title = 'Median Income') -> mapIncome 
mapIncome
...
dt <- dt %>% select(State, `Louisiana`, `Alabama`, `Mississippi`, `Georgia`, `South Carolina`) %>% 
  gather('deep south state', 'share', -State, -PVI)
...
mutate(income, state = tolower(state)) %>% 
  right_join(states, by = c('state' = 'region')) %>% 
      ...
      geom_polygon(aes(x = long, y = lat, group = group, fill = income), color = "white") + 
      coord_fixed(1.3) +
      labs(title = 'Median Income for Deep South') -> mapIncomeDeepSouth 
mapIncomeDeepSouth
```
...
`r read.csv(dt, row.names = paste0("Lousiana"), column.names = paste1("state", "median_house_inc", "gini_index"))`
